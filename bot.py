...
# Gamma markets patch disabled.
# The installed NautilusTrader version does not expose gamma_markets anymore.
print("Skipping gamma_market patch")
...
class IntegratedBTCStrategy(Strategy):
    ...
    def __init__(self, redis_client=None, enable_grafana=True, test_mode=False, recommend_only=False):
        super().__init__()
        ...
        self._current_market_question: str = ""
    ...
    def _load_all_btc_instruments(self):
        ...
        for instrument in instruments:
            try:
                if hasattr(instrument, 'info') and instrument.info:
                    question_raw = instrument.info.get('question', '')
        ...
        for i, inst in enumerate(btc_instruments):
            ...
            if is_active:
                ...
                self._current_market_question = inst.get('market_question', '')
                ...
                logger.info(f"  Question: {self._current_market_question}")
    ...
    def _switch_to_next_market(self):
        ...
        self._current_market_question = next_market.get('market_question', '')
    ...
    async def _make_trading_decision(self, current_price: Decimal):
        ...
        # --- Phase 5 / 6: Recommend or Execute ---
        if self.recommend_only:
            await self._record_contract_recommendation(
                signal=fused,
    ...
    async def _record_contract_recommendation(self, signal, position_size, current_price, direction, last_tick=None, trend_confidence=None):
        """Log the exact contract/side to buy without placing or paper-recording a trade."""
        import json

        market_question = getattr(self, '_current_market_question', '') or 'Unknown market'

        if direction == "long":
            contract = "YES"
            label = "YES / UP"
            instrument_id = str(getattr(self, '_yes_instrument_id', self.instrument_id))
        else:
            contract = "NO"
            label = "NO / DOWN"
            instrument_id = str(getattr(self, '_no_instrument_id', None))

        bid = ask = None
        if last_tick:
            bid, ask = last_tick

        # This is the max limit price you would be willing to pay right now.
        # For YES, use the YES ask. For NO, this repo's Polymarket adapter may expose
        # the active token book differently, so we keep current_price + bid/ask visible.
        suggested_limit_price = float(ask) if ask is not None else float(current_price)

        # Get the current market slug for reference
        current_slug = ''
        if self.all_btc_instruments and 0 <= self.current_instrument_index < len(self.all_btc_instruments):
            current_slug = self.all_btc_instruments[self.current_instrument_index].get('slug', '')

        rec = {
            "timestamp_utc": datetime.now(timezone.utc).isoformat(),
            "mode": "RECOMMEND_ONLY",
            "action": "BUY_LIMIT_CONTRACT",
            "contract": contract,
            "label": label,
            ...
            "reference_market_price": float(current_price),
            ...
            "trend_confidence": float(trend_confidence) if trend_confidence is not None else None,
            "signal_score": float(getattr(signal, 'score', 0.0)),
            "signal_confidence": float(getattr(signal, 'confidence', 0.0)),
        }

        # =====================================================================
        # Human-readable recommendation output
        # e.g. ">>> BUY the YES contract on 'Will BTC be above $103,500?'"
        # =====================================================================
        logger.info("")
        logger.info("=" * 80)
        logger.info("  RECOMMENDATION")
        logger.info("=" * 80)
        logger.info(f"  >>> BUY the {contract} contract on: {market_question}")
        logger.info(f"  Limit price: ${suggested_limit_price:.4f}")
        if bid is not None and ask is not None:
            logger.info(f"  Current bid/ask: ${float(bid):.4f} / ${float(ask):.4f}")
        logger.info(f"  Confidence: {float(trend_confidence or 0):.0%}")
        logger.info(f"  Signal: score={rec['signal_score']:.1f}  confidence={rec['signal_confidence']:.0%}")
...
def run_integrated_bot(simulation: bool = False, enable_grafana: bool = True, test_mode: bool = False, recommend_only: bool = False):
    ...
    print(f"\nConfiguration:")
    print(f"  Initial Mode: {'RECOMMEND ONLY' if recommend_only else ('SIMULATION' if simulation else 'LIVE TRADING')}")
...
def main():
    ...
    parser.add_argument("--recommend-only", action="store_true",
                        help="Only print/save the limit contract recommendation. Never submit or paper-record trades.")
    ...
    # --recommend-only and --test-mode ALWAYS force simulation even if --live is also passed
    ...
    if recommend_only:
        logger.info("=" * 80)
        logger.info("RECOMMEND-ONLY MODE — will print/save BUY LIMIT contract suggestions only.")
        logger.info("No real orders or paper trades will be placed.")
        logger.info("=" * 80)
    elif not simulation:
        logger.warning("=" * 80)
    ...
    run_integrated_bot(simulation=simulation, enable_grafana=enable_grafana, test_mode=test_mode, recommend_only=recommend_only)
